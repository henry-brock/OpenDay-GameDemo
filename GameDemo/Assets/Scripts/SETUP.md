...
- `TitleScreenController.cs` — title screen menu (Start New Game / Resume / Quit).
- `PauseMenu.cs` — in-level pause menu (Resume / Save / Return to Title Screen),
  opened with Escape or the gamepad Start button.
- `SaveSystem.cs` — PlayerPrefs-backed save/load powering the Resume flow.

All scenes (title screen + all three levels) are procedurally generated by
`Assets/Editor/LevelBuilder.cs` — use the **OpenDay** menu in the Unity Editor
(**Build Levels**, **Build Title Screen**) rather than building menus by hand.

## Music

Each scene has its own music, picked up **by folder name** — there is nothing to
wire up in the Inspector, and your choices survive the scenes being rebuilt.

1. Drop audio files (`.wav`, `.mp3`, `.ogg`) into the matching folder:
   - `Assets/Audio/TitleScreen/`
   - `Assets/Audio/Level1_ObjectOrientedProgramming/`
   - `Assets/Audio/Level2_SoftwareEngineering/`
